...
- `focusFieldWithin` — the same, for a container that wraps a borderless field, such as a composer around a textarea. It lights while the field has focus, not while a button inside the container does.
